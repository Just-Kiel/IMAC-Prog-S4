...
void ShadowMapping::render(Model& casterModel, const std::vector<ModelParams>& castersParams, const VBO& vboInstancing, const glm::mat4& ProjMatrix, const glm::mat4& lightSpaceMatrix)
{
    if (!m_displayShadow)
        return;
    ...
    casterModel.drawInstancedModel(m_shader, ProjMatrix, lightSpaceMatrix, castersParams, vboInstancing);
